 ...
 * through THE single Circle Gateway wrapper, then reflected as `float_pending` ONLY — pending float is
 ...
 *
 * Custody note (reconciliation, no spec fork): the treasury is Circle-custodied (developer-controlled
 * Gateway), so `gateway.depositFor` is API-authorized — there is NO client-side EIP-712 signature on this
 * path. The treasury-allocation / internal-allocation KMS fence (`signer.ts`) governs the self-custody
 * rail, where agentOps itself signs. The external-redirect threat is closed here by the own-agent
 * destination fence in `evaluateAllocation` plus Circle's custodial deposit-for endpoint.
 */
...
export async function depositFor(
  ...
  let txRef: string;
  try {
    const op = await gateway.depositFor({ orgId, agentId, amount });
    txRef = op.id;
  } catch (err) {
