...
import { revokeWasmBase64 } from '../../../src/engines/identity/delegation/grant-wasm.js';
...
async function createAgentWithKey(app: FastifyInstance, cookie: string): Promise<{ agentId: string; delegatedPublicKey: string }> {
  const res = await app.inject({
    ...
    url: '/v1/agents',
    headers: { cookie },
    payload: { name: 'revoke-init-agent' },
  });
...
describe('POST /v1/agents/:id/revoke-delegated-key/init', () => {
  it('200: returns unsigned revoke + wasm_base64 + chain_name; server signs nothing', async ({ skip }) => {
    if (!dockerAvailable || !pool || !redis) return skip();
    const app = buildAppWithVault(pool, redis);
    try {
      const { cookie } = await seedUserOrgOwner(pool, 'revoke-init-owner@test.com');
      const { agentId, delegatedPublicKey } = await createAgentWithKey(app, cookie);

      const res = await app.inject({
        method: 'POST',
        url: `/v1/agents/${agentId}/revoke-delegated-key/init`,
        headers: { cookie },
      ...
      expect(body.unsigned_revoke.agent_account_hash).toBe(accountHashFromPublicKeyHex(delegatedPublicKey));
      expect(body.unsigned_revoke.master_account_hash).toBe(accountHashFromPublicKeyHex(MASTER_PUBLIC_KEY));
      expect(body.unsigned_revoke.args).toEqual({
      ...
      expect(createHash('sha256').update(wasm).digest('hex')).toBe(
        createHash('sha256').update(Buffer.from(revokeWasmBase64(), 'base64')).digest('hex'),
      );
  ...
  it('409 no_delegated_key when the agent has no active delegated key', async ({ skip }) => {
    ...
    const app = buildAppWithVault(pool, redis);
    try {
      const { cookie } = await seedUserOrgOwner(pool, 'revoke-init-nokey@test.com');
  ...
  it('404 agent_not_found for an agent owned by a different org', async ({ skip }) => {
    ...
    const app = buildAppWithVault(pool, redis);
    try {
      const owner = await seedUserOrgOwner(pool, 'revoke-init-org-a@test.com');
      const { agentId } = await createAgentWithKey(app, owner.cookie);
      const other = await seedUserOrgOwner(pool, 'revoke-init-org-b@test.com');
  ...
  it('400 invalid_body when master_public_key is missing or not hex', async ({ skip }) => {
    ...
    const app = buildAppWithVault(pool, redis);
    try {
      const { cookie } = await seedUserOrgOwner(pool, 'revoke-init-badbody@test.com');
      const { agentId } = await createAgentWithKey(app, cookie);
